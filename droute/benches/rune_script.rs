...
use droute::{builders::*, mock::Server, AsyncTryInto, Router, ScriptBackend, ScriptBuilder};
...
fn bench_resolve(c: &mut Criterion) {
    ...
    let router = rt.block_on(create_router(RuneScriptBuilder::new(
        r#"pub async fn route(upstreams, inited, ctx, query) { upstreams.send("mock", CacheMode::Disabled, query).await } pub async fn init() { Ok(#{}) }"#,
    )));
    let cached_router = rt.block_on(create_router(RuneScriptBuilder::new(
        r#"pub async fn route(upstreams, inited, ctx, query) { upstreams.send_default("mock", query).await } pub async fn init() { Ok(#{}) }"#,
    )));

    c.bench_function("rune_non_cache_resolve", |b| {
    ...
    c.bench_function("rune_cached_resolve", |b| {
        b.to_async(&rt).iter(|| async {
            assert_eq!(
                cached_router
                    .resolve(QUERY.clone(), None)
